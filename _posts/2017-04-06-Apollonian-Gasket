...
----------------------------------------
In either paradigm, the first stage of the gasket involves plotting \f$n_{zero}\f$ circles, the first of which contains all of the circles within 
the structure. The radius of the 0th circle on the 0th level of the gasket may be specified or may without loss of generality be assumed to be equal to 1. 
Assume that the 0th circle on the 0th level of the gasket has radius \f$R\f$. The remaining \f$n_{zero}-1\f$ consecutive circles on the 0th level 
are tangent to each other and the circle that contains them. In fact, each of the \f$n_{zero}-1\f$ circles inhabit contiguous sectors 
of the 0th circle; the angle defining each of the sectors is given by:
\f{align*}{
...
The line bisecting the circle makes an angle of \f$\alpha\f$ with the first line that defines the sector. Defining the radius of the first circle as
\f$r\f$, it follows from trigonometric identities that:
\f{align*}{
...
\f}
The practical implication is that after any line defining the radius \f$R\f$ of the 0th circle is chosen, the center of the first circle 
is positioned a distance of \f$R-r\f$ units away from the center of the 0th circle. The first circle has been shown to have radius \f$r\f$. 
The remaining \f$n_{zero}-2\f$ circles on the 0th level are given by rotating the first circle by an angle of \f$2\alpha\f$ 
consecutive times; each consecutive circle has a radius of \f$r\f$. The figure below displays the 0th level of a gasket specified with \f$n_{zero}=4\f$. 
The center of the 0th circle is the origin, and the center of the first circle on the 0th level is given by the point \f$\left(0,R-r\right)\f$ (this 
direction was arbitrary).
<img src="../lvl0.png" width="500" height="500" />
Constructing the reference circles
-----------------------------------
Once the 0th level of the gasket has been populated, \f$n_{zero}\f$ reference circles are defined, which form a crucial part of the algrithm for creating Apollonian 
gaskets. The 0th reference circle passes through the each point of tangency of the inner circles \f$1,\ldots,n_{zero}-1\f$. It is sufficient to 
